...
import { deleteById, getAllMessages, getMessageById, sendMessage, toggleFavourite } from './message.service.js'
...
router.patch('/favourite/:id',
    authentication(),
    validation(validators.getMessageSchema),
    async(req, res, next) => {
        const message = await toggleFavourite(req.params.id, req.user);
        return successResponse({ res, result: { message } });
    }
);
