...
export const coverPicture = async (files, user) => {
    ...
    if (!files || !files.length) {
        throw BadRequestException({ message: "Value Are Required ❌" });
    }

    if (files.length > 2) {
        throw BadRequestException({ message: "Max 2 images only" });
    }

    const newPaths = files.map(file => file.finalPath);

    // 🔥 هنا الحل الحقيقي (REPLACE NOT APPEND)
    user.coverProfilePicture = newPaths;
...
//         throw BadRequestException({message:"Value Are Required ❌"})
//     }
//     const existingCount = user.coverProfilePicture?.length || 0
//     const newCovers = files.length
//     const totalImages = existingCount +newCovers;

//     if (totalImages > 2) {
//         throw BadRequestException({message : "Total cover images  cannot exceed 2"})
//     }
//     const newPaths = files.map(file => file.finalPath)
//     user.coverProfilePicture =[
//             ...(user.coverProfilePicture || [] ),
//             ...newPaths
//     ]
//     await user.save()
