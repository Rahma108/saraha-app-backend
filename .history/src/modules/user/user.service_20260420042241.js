...
export const coverPicture = async (files, user) => {
    ...
    const existingImages = user.coverProfilePicture || [];
    const newImages = files?.coverProfilePicture ?? [];
    ...
    if (totalImages > 2) {
        throw new Error("Max 2 cover images only");
    }

    const mapped = newImages.map(file => ({
        finalPath: file.finalPath || file.filename || file.path
    }));
